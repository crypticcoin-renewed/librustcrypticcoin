...
impl JsDescription {
    pub fn read<R: Read>(mut reader: R, use_groth: bool) -> io::Result<Self> {
        ...
        // enforced by CheckTransactionWithoutProofVerification() in crypticcoind.
        ...
        // ZCNoteDecryption::decrypt() in crypticcoind
        ...
        let proof = if use_groth {
            // Consensus rules (§4.3):
            // - Canonical encoding is enforced in librustcrypticcoin_sprout_verify()
            // - Proof validity is enforced in librustcrypticcoin_sprout_verify()
            let mut proof = [0u8; GROTH_PROOF_SIZE];
            reader.read_exact(&mut proof)?;
            SproutProof::Groth(proof)
        } else {
            // Consensus rules (§4.3):
            // - Canonical encoding is enforced by PHGRProof in crypticcoind
            // - Proof validity is enforced by JSDescription::Verify() in crypticcoind
