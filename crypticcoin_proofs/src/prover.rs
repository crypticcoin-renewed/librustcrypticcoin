...
impl LocalTxProver {
    ...
    ///     Some(tx_prover) => (),
    ///     None => println!("Please run crypticcoin-fetch-params or fetch-params.sh to download the parameters."),
    /// }
    /// ```
    ///
    /// # Panics
    ...
    pub fn bundled() -> Self {
        let (spend_buf, output_buf) = wagyu_crypticcoin_parameters::load_sapling_parameters();
