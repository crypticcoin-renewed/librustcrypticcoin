...
pub fn parse_parameters<R: io::Read>(
    ...
    if spend_fs.into_hash() != SAPLING_SPEND_HASH {
        panic!("Sapling spend parameter file is not correct, please clean your `~/.crypticcoin-params/` and re-run `fetch-params`.");
    }

    if output_fs.into_hash() != SAPLING_OUTPUT_HASH {
        panic!("Sapling output parameter file is not correct, please clean your `~/.crypticcoin-params/` and re-run `fetch-params`.");
    }
    ...
    if sprout_fs
        ...
        .unwrap_or(false)
    {
        panic!("Sprout groth16 parameter file is not correct, please clean your `~/.crypticcoin-params/` and re-run `fetch-params`.");
    }
